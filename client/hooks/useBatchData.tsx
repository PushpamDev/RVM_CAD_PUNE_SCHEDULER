...
export function useBatchData() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [batchesRes, facultiesRes, studentsRes, skillsRes] = await Promise.all([
        fetch(`${API_BASE_URL}/api/batches?include=skill,faculty`, { headers: { Authorization: `Bearer ${token}` } }),
